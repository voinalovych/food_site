document.addEventListener('DOMContentLoaded', () => {
	...
	// window.addEventListener('scroll', showModalByScroll); // !!! Выключен !!!
	...
	// const getResource = async (url) => {  // до axios
	...
	// getResource(`http://localhost:3000/menu`)      // до axios 
	// 	.then(data => {
	...
	axios.get('http://localhost:3000/menu')
		.then(menuData => {
			menuData.data.forEach(({img, altimg, title, descr, price}) => {
				new MenuCard(img, altimg, title, descr, price, '.menu .container').render();
			});
		});
	...
	function bindPostData(form) {
		form.addEventListener('submit', (e) => {
			...
			// const request = new XMLHttpRequest();	//поменяли на fetch
			...
			// request.send(json); // было formData

			// fetch('server.php', {			//вынесли в отдельную f
			// 	method: "POST",
			// 	headers: {'Content-type':'application/json; charset=utf-8'},
			// 	// body: formDatassss
	...
	// 	// current.innerHTML = addZero(index);
